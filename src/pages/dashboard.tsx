...
import { getMarkets } from 'apiSdk/markets';
import { MarketInterface } from 'interfaces/market';
import { getTrades } from 'apiSdk/trades';
...
import { AnalystListPage as ShowCaseList } from 'pages/analysts';

function DashboardPage() {
  const { session } = useSession();

  const marketStat = useSWR<PaginatedInterface<MarketInterface>>(
    () => `/markets`,
    () => getMarkets({ limit: 1, offset: 0 }),
  );

  const tradeStat = useSWR<PaginatedInterface<TradeInterface>>(
    () => `/trades`,
    () => getTrades({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Markets',
      result: marketStat?.data?.totalCount,
      isLoading: marketStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Trades',
      result: tradeStat?.data?.totalCount,
      isLoading: tradeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Owners',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`analysts`} passHref legacyBehavior>
                  <Button
