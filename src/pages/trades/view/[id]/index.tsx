            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('trade', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/trades/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Trade Type" text={data?.trade_type} />

                <FormListItem
                  label="Trade Date"
                  text={data?.trade_date ? format(parseISO(data?.trade_date as unknown as string), 'dd-MM-yyyy') : ''}
                />
                ...
                {hasAccess('market', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Market"
                    text={
                      <Link as={NextLink} href={`/markets/view/${data?.market?.id}`}>
                        {data?.market?.name}
                      </Link>
                    }
                  />
