...
type ColumnType = ColumnDef<TradeInterface, unknown>;
...
export function TradeListPage(props: TradeListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getTrades({
        ...
        order: params.order,
        searchTermKeys: ['trade_type.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TradeInterface>>(
    () => `/trades?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TradeInterface) => {
    if (hasAccess('trade', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/trades/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'trade_type', header: 'Trade Type', accessorKey: 'trade_type' },
    {
      id: 'trade_date',
      header: 'Trade Date',
      accessorKey: 'trade_date',
      cell: ({ row: { original: record } }: any) =>
        record?.trade_date ? format(parseISO(record?.trade_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    ...
    hasAccess('market', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'market',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/markets/view/${record.market?.id}`}>
              {record.market?.name}
            </Link>
          ),
        }
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/trades/view/${record.id}`} passHref legacyBehavior>
                <Button
